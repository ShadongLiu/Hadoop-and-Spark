    ...
    textFile
      .flatMap(line => {
        ...
        if (tokens.length > 1) {
          ...
          val wordmarginal = tokens.init.map(word => (word, "*")).toList
          //concatenation of list of pairs and list of marginal values
          pairs ++ wordmarginal
        }else List()
      })
      //
      .map(pairs => (pairs, 1.0f))
      //(x,y) => x + y
      .reduceByKey(_ + _)
      //sort to ensure that (A, "*") appears before other (A, otherword) pairs
      .sortByKey().partitionBy(new HashPartitioner(args.reducers()))
      //follow the logic in java
      .map(pairs => pairs._1 match{
        case (_,"*") => {
          marginal = pairs._2
          (pairs._1, pairs._2)
        }
        case (_,_) => {
          (pairs._1, pairs._2/marginal)
        }
      })
